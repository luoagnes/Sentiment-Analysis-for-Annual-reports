...
import os
import re
from config import ini_CS_path, filtered_CS_path
...
def split_chunk(txt_path, txt_name, target_path):
    ...
    if len(result_content.strip())>0:
        with open(target_path+txt_name[:10]+".txt", "w") as txt_f:
...
if __name__ == '__main__':
    ...
    if not os.path.exists(filtered_CS_path):
        os.mkdir(filtered_CS_path)
        
    process(ini_CS_path, filtered_CS_path)
