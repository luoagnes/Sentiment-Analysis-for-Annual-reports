    ...
    for s in published_date.keys():
		...
		for y in published_date[s].keys():
			...
			if next_year in published_date[s].keys():
				...
				result[s][y]=get_one_year_data(start_y, end_y, stock_price)
...
def main():
	...
	## 3. stock price
	price_file=price_data_path+'Close_Price.json'
	stock_price=json.loads(open(price_file, 'r').read())

	## 4. get one year data
	get_all_year_data(published_date, stock_price, result_path)
	
	## 5. regression
	result=get_all_gradient(result_path)
